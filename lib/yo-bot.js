...
YoBot.prototype._firstRunCheck = function() {
  this._greetNewUser('datatypevoid');
  var self = this;
...
YoBot.prototype._getStartupMessage = function() {
  return fs.readFileSync('data/startup.txt', 'utf8');
};

YoBot.prototype._getWelcomeMessage = function() {
  return fs.readFileSync('data/first-start.txt', 'utf8');
};

YoBot.prototype._fetchGreeting = function() {
  return fs.readFileSync('welcome.txt', 'utf8');
};

YoBot.prototype._greetNewUser = function(username) {
  var self = this;
  var greeting = self._fetchGreeting();
  setTimeout(function() {
...
YoBot.prototype._reply = function (originalMessage) {
  ...
  var placeholder = 'Additional functionality is in the works. Please contact ' +
                    '<@U0KALLQTW|datatypevoid> for any assistance you may require.';
...
YoBot.prototype._startupMessage = function() {
  var startUpMessage = this._getStartupMessage();
  this.postMessageToChannel(this.channels[0].name, startUpMessage, { as_user: true });
};
